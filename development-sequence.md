...
| # | Functionality | Depends on | Reference |
|---|---|---|---|
| 5.1 | AR-X via conditional least squares | 1.4 | `stats::ar()` (R) · `statsmodels.tsa.ar_model.AutoReg` (Python) |
| 5.2 | Generic forecast object + prediction intervals | 5.1 (first model to attach it to) | `forecast` S3 class (R) · `get_prediction` (Python) |
| 5.3 | Accuracy metrics — MAE, RMSE, MAPE, MASE, sMAPE | 5.2 | `forecast::accuracy` (R) · `sktime.performance_metrics` (Python) |
| 5.4 | Rolling-origin time series cross-validation | 5.2 | `forecast::tsCV` (R) · `sktime.split.ExpandingWindowSplitter` (Python) |
| 5.5 | Simple / Holt / Holt-Winters (classical SSE fitting) *(was 8.1)* | 4.1 only — doesn't need the Kalman engine at all | Base `HoltWinters()` (R) · `statsmodels.tsa.holtwinters.ExponentialSmoothing` (Python) |
