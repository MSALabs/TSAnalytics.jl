...
# verification/bulk/gen_cases.py + run_r.R + merge_and_emit.py.
#
...
#
# Regenerate with:
#   python verification/bulk/gen_cases.py
#   Rscript verification/bulk/run_r.R   # or the full path to Rscript.exe
#   python verification/bulk/merge_and_emit.py

using Test, DelimitedFiles, LinearAlgebra

@testset "GaussianSSM bulk -- dual-verified R+Python, real+synthetic datasets" begin

    y_ar1_shared = vec(readdlm(joinpath(@__DIR__, "../verification/bulk/data/ar1_shared.csv"), ',', skipstart=1))
    y_ar2_shared = vec(readdlm(joinpath(@__DIR__, "../verification/bulk/data/ar2_shared.csv"), ',', skipstart=1))
    y_co2_diff = vec(readdlm(joinpath(@__DIR__, "../verification/bulk/data/co2_diff.csv"), ',', skipstart=1))
    y_elnino_monthly = vec(readdlm(joinpath(@__DIR__, "../verification/bulk/data/elnino_monthly.csv"), ',', skipstart=1))
    y_ma1_shared = vec(readdlm(joinpath(@__DIR__, "../verification/bulk/data/ma1_shared.csv"), ',', skipstart=1))
    y_macro_gdp_growth = vec(readdlm(joinpath(@__DIR__, "../verification/bulk/data/macro_gdp_growth.csv"), ',', skipstart=1))
    y_nile = vec(readdlm(joinpath(@__DIR__, "../verification/bulk/data/nile.csv"), ',', skipstart=1))
    y_sunspots = vec(readdlm(joinpath(@__DIR__, "../verification/bulk/data/sunspots.csv"), ',', skipstart=1))

    let ar = [0.3], ma = Float64[], sar = Float64[], sma = Float64[], s = 0
        ar_c, ma_c = combined_ar_ma(ar, sar, ma, sma, max(s, 1))
        ssm = build_statespace(ar_c, ma_c)
        loglik, sigma2, v, F, converged = kalman_filter(ssm, y_ar1_shared)
    ...
    let ar = [0.3], ma = Float64[], sar = Float64[], sma = Float64[], s = 0
        ar_c, ma_c = combined_ar_ma(ar, sar, ma, sma, max(s, 1))
        ssm = build_statespace(ar_c, ma_c)
        loglik, sigma2, v, F, converged = kalman_filter(ssm, y_ar2_shared)
    ...
    let ar = [0.3], ma = Float64[], sar = Float64[], sma = Float64[], s = 0
        ar_c, ma_c = combined_ar_ma(ar, sar, ma, sma, max(s, 1))
        ssm = build_statespace(ar_c, ma_c)
        loglik, sigma2, v, F, converged = kalman_filter(ssm, y_ma1_shared)
    ...
    let ar = [0.3], ma = Float64[], sar = Float64[], sma = Float64[], s = 0
        ar_c, ma_c = combined_ar_ma(ar, sar, ma, sma, max(s, 1))
        ssm = build_statespace(ar_c, ma_c)
        loglik, sigma2, v, F, converged = kalman_filter(ssm, y_nile)
    ...
    let ar = [0.3], ma = Float64[], sar = Float64[], sma = Float64[], s = 0
        ar_c, ma_c = combined_ar_ma(ar, sar, ma, sma, max(s, 1))
        ssm = build_statespace(ar_c, ma_c)
        loglik, sigma2, v, F, converged = kalman_filter(ssm, y_sunspots)
    ...
    let ar = [0.3], ma = Float64[], sar = Float64[], sma = Float64[], s = 0
        ar_c, ma_c = combined_ar_ma(ar, sar, ma, sma, max(s, 1))
        ssm = build_statespace(ar_c, ma_c)
        loglik, sigma2, v, F, converged = kalman_filter(ssm, y_macro_gdp_growth)
    ...
    let ar = [0.3], ma = Float64[], sar = Float64[], sma = Float64[], s = 0
        ar_c, ma_c = combined_ar_ma(ar, sar, ma, sma, max(s, 1))
        ssm = build_statespace(ar_c, ma_c)
        loglik, sigma2, v, F, converged = kalman_filter(ssm, y_elnino_monthly)
    ...
    let ar = [0.3], ma = Float64[], sar = Float64[], sma = Float64[], s = 0
        ar_c, ma_c = combined_ar_ma(ar, sar, ma, sma, max(s, 1))
        ssm = build_statespace(ar_c, ma_c)
        loglik, sigma2, v, F, converged = kalman_filter(ssm, y_co2_diff)
