...
driver over `_stl_est1`, evaluating it at every point in `xout`.
...
parallel piece of this loop, see `_stl_cycle_subseries!`) uses
...
- `parallel`: threads the cycle-subseries smoothing step (the `period`
  independent per-phase Loess fits -- see `_stl_cycle_subseries!`) via
  `Threads.@threads`, the *only* genuinely parallel piece of STL --
  everything else (the `inner`/outer
