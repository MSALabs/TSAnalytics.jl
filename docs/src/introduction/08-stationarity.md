...
y = iip.value
growth = diff(log.(y))
adf_level = adf_test(y)
adf_growth = adf_test(growth)
println("iip_india, level:  ADF statistic=", round(adf_level.statistic,digits=3), "  p=", round(adf_level.pvalue,digits=4))
