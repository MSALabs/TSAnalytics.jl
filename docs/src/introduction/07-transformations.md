...
y = iip.value
w = 24
means = [mean(y[i:i+w-1]) for i in 1:(length(y)-w+1)]
sds = [std(y[i:i+w-1]) for i in 1:(length(y)-w+1)]
lmeans = [mean(log.(y)[i:i+w-1]) for i in 1:(length(y)-w+1)]
lsds = [std(log.(y)[i:i+w-1]) for i in 1:(length(y)-w+1)]
println("rolling 24-month mean-vs-sd correlation, level:     ", round(cor(means, sds), digits=3))
